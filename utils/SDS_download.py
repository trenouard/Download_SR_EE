        ...
        for i in range(len(im_dict_T1[satname])):
            ...
            if satname == 'L5':
                bands[''] = [im_bands[0], im_bands[1], im_bands[2], im_bands[3],
                             im_bands[4], im_bands[8], im_bands[17]]
            ...
            # Landsat 7 and 8 download
                ...
                if satname == 'L7':
                    ...
                    bands[''] = [im_bands[1], im_bands[2], im_bands[3], im_bands[4],
                                   im_bands[5], im_bands[8], im_bands[17]]
                im_fn[''] = im_date + '_' + satname + '_' + inputs['sitename'] + suffix
                # if two images taken at the same date add 'dup' to the name (duplicate)
                if any(im_fn[''] in _ for _ in all_names):
                    im_fn[''] = im_date + '_' + satname + '_' + inputs['sitename'] + '_dup' + suffix
...
def check_images_available(inputs):
    """
    Create the structure of subfolders for each satellite mission and check the number of images available
    ...
    col_names_T1 = {'L5':'LANDSAT/LT05/C02/T1_L2',
                    'L7':'LANDSAT/LE07/C02/T1_L2',
                    'L8':'LANDSAT/LC08/C02/T1_L2',
                    'S2':'COPERNICUS/S2_SR'}

    print('- In Landsat Tier 1 Level 2 & Sentinel-2 Level-2A:')
    im_dict_T1 = dict([])
    sum_img = 0
    ...
    col_names_T2 = {'L5':'LANDSAT/LT05/C02/T2_L2',
                 'L7':'LANDSAT/LE07/C02/T2_L2',
                 'L8':'LANDSAT/LC08/C02/T2_L2'}
